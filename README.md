...
交易池负责维护尚未确认的交易。为了避免打包重复的交易，本实现使用 IndexSet(保留插入顺序的hashset) 来保存这些交易的hash，通过 IndexSet 也实现了在打包交易的时候使用先进先出的策略。

交易池直到块最终确认（后面累计 block_delay_number 个后续块），才会将交易从交易池中删除。

为了避免当交易池中交易数量过多时, 导致新进入的交易无法被打包(触发 valid_until_block 时), 通过 node_status 中的 waiting_block 来标识当前节点等待处理中的块数量。

#### auditor

auditor主要负责检查交易的合法性，包括交易中每个字段的合法性，以及与历史交易是否重复。auditor会保存最近100个历史块中的交易hash。
