...
impl Auditor {
    ...
    pub fn auditor_check(&self, raw_tx: &RawTransaction) -> Result<(), StatusCodeEnum> {
    ...
    pub fn auditor_check_batch(&self, raw_txs: &RawTransactions) -> Result<(), StatusCodeEnum> {
        for raw_tx in raw_txs.body.as_slice() {
            self.auditor_check(raw_tx)?;
        }
        Ok(())
    }
}
