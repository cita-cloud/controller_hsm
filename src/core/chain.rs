...
use crate::{
    ...
    protocol::node_manager::ChainStatus,
    util::*,
};
...
impl Chain {
    ...
    pub async fn init_auditor(&self, init_block_number: u64) {
        let mut auditor = self.auditor.write().await;
        auditor.init(init_block_number).await;
    }
    ...
    pub async fn get_system_config(&self) -> SystemConfig {
        let rd = self.auditor.read().await;
